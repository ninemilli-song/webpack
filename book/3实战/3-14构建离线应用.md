...
<li>[Service Workers](https://developer.mozilla.org/zh-CN/docs/Web/API/Service_Worker_API/Using_Service_Workers) 是目前最新的离线缓存技术，是 [Web Worker](http://javascript.ruanyifeng.com/htmlapi/webworker.html) 的一部分。
它通过拦截网络请求实现离线缓存，比 AppCache 更加灵活。它也是构建 [PWA](https://developer.mozilla.org/zh-CN/Apps/Progressive) 应用的关键技术之一。</li>
...
```
if (navigator.serviceWorker) {
  window.addEventListener(&amp;aposDOMContentLoaded&amp;apos,function() {
    // 调用 serviceWorker.register 注册，参数 /sw.js 为脚本文件所在的 URL 路径
      navigator.serviceWorker.register(&amp;apos/sw.js&amp;apos);
  });
}
...
// 需要被缓存的文件的 URL 列表
var cacheFileList = [
  &amp;apos/index.html&amp;apos,
  &amp;apos/app.js&amp;apos,
  &amp;apos/app.css&amp;apos
];

// 监听 install 事件
self.addEventListener(&amp;aposinstall&amp;apos, function (event) {
...
```
self.addEventListener(&amp;aposfetch&amp;apos, function(event) {
  event.respondWith(
    // 去缓存中查询对应的请求
...
1. 新 Service Workers 线程将会启动，且将会触发其 install 事件。
1. 当网站上当前打开的页面关闭时，旧 Service Workers 线程将会被终止，新 Service Workers 线程将会取得控制权。
1. 新 Service Workers 线程取得控制权后，将会触发其 activate 事件。
...
self.addEventListener(&amp;aposactivate&amp;apos, function(event) {
  event.waitUntil(
...
并且`sw.js`文件中的 `cacheFileList` 变量，代表需要被缓存文件的 URL 列表，需要根据输出文件列表所对应的 URL 来决定，而不是像上面那样写成静态值。
...
var cacheFileList = [
  &amp;apos/index.html&amp;apos,
  &amp;aposapp_4c3e186f.js&amp;apos,
  &amp;aposapp_7cc98ad0.css&amp;apos
];
...
const ServiceWorkerWebpackPlugin = require(&amp;aposserviceworker-webpack-plugin&amp;apos);

module.exports = {
  entry: {
    app: &amp;apos./main.js&amp;apos// Chunk app 的 JS 执行入口文件
  },
  output: {
    filename: &amp;apos[name].js&amp;apos,
    publicPath: &amp;apos&amp;apos,
  },
  module: {
    rules: [
      {
        ...
        use: ExtractTextPlugin.extract({
          use: [&amp;aposcss-loader&amp;apos] // 压缩 CSS 代码
        }),
      },
    ]
  },
  plugins: [
    ...
    new ServiceWorkerWebpackPlugin({
      ...
      entry: path.join(__dirname, &amp;apossw.js&amp;apos),
    }),
  ],
  devServer: {
...
<img alt="图3.12.1 查看当前页面注册的 Service Workers" src="img/3-14service-workers.png"/>

通过打开开发者工具的 Application-Cache-Cache Storage 一栏，能看到当前页面缓存的资源列表，正常的效果如图：

<img alt="图3.12.2 查看当前页面的 Cache Storage" src="img/3-14cache-storage.png"/>

为了验证网页在离线时能访问的能力，需要在开发者工具中的 Network 一栏中通过 Offline 选项禁用掉网络，再刷新页面能正常访问，并且网络请求的响应都来自 Service Workers，正常的效果如图：

<img alt="图3.12.3 离线情况下访问页面" src="img/3-14sw-offline.png"/>
