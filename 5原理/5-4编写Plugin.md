...
class BasicPlugin{
  ...
  apply(compiler){
    compiler.plugin(&amp;aposcompilation&amp;apos,function(compilation) {
    })
  }
}
...
```
const BasicPlugin = require(&amp;apos./BasicPlugin.js&amp;apos);
module.export = {
  plugins:[
...
* params 为附带的参数
*/
compiler.apply(&amp;aposevent-name&amp;apos,params);

/**
* 监听名称为 event-name 的事件，当 event-name 事件发生时，函数就会被执行。
* 同时函数中的 params 参数为广播事件时附带的参数。
*/
compiler.plugin(&amp;aposevent-name&amp;apos,function(params) {
...
<li>有些事件是异步的，这些异步的事件会附带两个参数，第二个参数为回调函数，在插件处理完任务时需要调用回调函数通知 Webpack，才会进入下一处理流程。例如：
<pre><code class="lang-js">  compiler.plugin(&amp;aposemit&amp;apos,function(compilation, callback) {
    // 支持处理逻辑
...
class Plugin {
  apply(compiler) {
    compiler.plugin(&amp;aposemit&amp;apos, function (compilation, callback) {
...
在[4-5使用自动刷新](../4优化/4-5使用自动刷新.html) 中介绍过 Webpack 会从配置的入口模块出发，依次找出所有的依赖模块，当入口模块或者其依赖的模块发生变化时，
...
compiler.plugin(&amp;aposwatch-run&amp;apos, (watching, callback) =&gt; {
    // 获取发生变化的文件列表
...
```
compiler.plugin(&amp;aposafter-compile&amp;apos, (compilation, callback) =&gt; {
...
```
compiler.plugin(&amp;aposemit&amp;apos, (compilation, callback) =&gt; {
  ...
  compilation.assets[fileName] = {
    ...
    // 返回文件大小
      size: () =&gt; {
      return Buffer.byteLength(fileContent, &amp;aposutf8&amp;apos);
    }
  };
  callback();
});
...
class EndWebpackPlugin {
  ...
  apply(compiler) {
    compiler.plugin(&amp;aposdone&amp;apos, (stats) =&gt; {
        // 在 done 事件中回调 doneCallback
        this.doneCallback(stats);
    });
    compiler.plugin(&amp;aposfailed&amp;apos, (err) =&gt; {
...
在 [5-1工作原理概括](5-1工作原理概括.html) 中详细介绍过 Webpack 在运行过程中广播出常用事件，你可以从中找到你需要的事件。
