...
```
const { app, BrowserWindow } = require(&amp;aposelectron&amp;apos)
...
app.on(&amp;aposready&amp;apos, createWindow)

// 当全部窗口关闭时退出
app.on(&amp;aposwindow-all-closed&amp;apos, () =&gt; {
  // 在 macOS 上，除非用户用 Cmd + Q 确定地退出
  // 否则绝大部分应用会保持激活
  if (process.platform !== &amp;aposdarwin&amp;apos) {
    app.quit()
  }
})
...
例如在 `app.on(&amp;aposready&amp;apos)` 事件中通过 `BrowserWindow` 去展示应用的主窗口，具体用法见 [BrowserWindow 的 API 文档](https://github.com/electron/electron/blob/master/docs-translations/zh-CN/api/browser-window.md)。

启动的窗口其实是一个网页，启动时会去加载在 `loadURL` 中传入的网页地址。
每个窗口都是一个单独的网页进程，窗口之间的通信需要借助主进程传递消息。

<img alt="Electron 应用架构图" src="img/3-12electron-arch.png"/>
...
也就是说项目由2个单页应用组成，这和[3-10管理多个单页应用](3-10管理多个单页应用.html) 中的项目非常相似，让我们来把它改造成一个 Electron 应用。
...
import { remote } from &amp;aposelectron&amp;apos;
import path from &amp;apospath&amp;apos;
...
class App extends Component {
  ...
  handleBtnClick() {
    // 新窗口对应的页面的 URI 地址
    const modalPath = path.join(&amp;aposfile://&amp;apos, remote.app.getAppPath(), &amp;aposdist/login.html&amp;apos);
    // 新窗口的大小
    let win = new remote.BrowserWindow({ width: 400, height: 320 })
    win.on(&amp;aposclose&amp;apos, function () {
      // 窗口被关闭时清空资源
      win = null
    })
    // 加载网页
...
```
target: &amp;aposelectron-renderer&amp;apos,
...
<img alt="Electron 应用运行效果图" src="img/3-12electron-app.png"/>
